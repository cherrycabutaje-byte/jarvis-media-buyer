...
import { runBuilderAndPersistAction } from "@/lib/actions/builderActions";
...
export default function BuildProductPage() {
  ...
  async function handleRun() {
    ...
    try {
      setLoading(true);
      setError("");
      const result = await runBuilderAndPersistAction(productId);
      if (!result.success) {
    ...
    <main style={{background: "#080b12", minHeight: "100vh"}}>
      ...
      <div style={{maxWidth: "440px", margin: "0 auto", padding: "60px 20px"}}>
        <div style={{marginBottom: "32px"}}>
          <h1 style={{fontSize: "32px", fontWeight: "800", color: "#ffffff", marginBottom: "12px"}}>Build Product</h1>
          <p style={{color: "#94a3b8", fontSize: "16px", lineHeight: "1.6"}}>Runs the Builder Layer (Product Structure, Package Definition, Decision Engine) against this product&apos;s Brain analysis and saves the result.</p>
        </div>

        {success ? (
          <div style={{background: "#0f172a", border: "1px solid #1e293b", borderRadius: "16px", padding: "24px", color: "#e2e8f0"}}>
            <p style={{color: "#22d3ee", fontWeight: "700", margin: 0}}>Product built successfully!</p>
          </div>
        ) : (
          <div style={{background: "#0f172a", border: "1px solid #1e293b", borderRadius: "16px", padding: "24px"}}>
            ...
            <button onClick={handleRun} disabled={loading} style={{width: "100%", padding: "14px", background: loading ? "#1e293b" : "#22d3ee", color: loading ? "#475569" : "#080b12", border: "none", borderRadius: "10px", fontWeight: "700", fontSize: "15px", cursor: loading ? "default" : "pointer"}}>
              {loading ? "Building..." : "Run Builder Layer"}
            </button>
          </div>
        )}
      </div>
